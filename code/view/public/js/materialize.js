    ...
    _createClass(Autocomplete, [{
      ...
      value: function _renderDropdown(data, val) {
        ...
        for (var i = 0; i < matchingData.length; i++) {
          ...
          if (!!_entry.public_process) {
            $autocompleteOption.append("<img src=\"" + _entry.public_process + "\" class=\"right circle\"><span>" + _entry.key + "</span>");
          } else {
      ...
      value: function open() {
        ...
        if (val.length >= this.options.minLength) {
          this.isOpen = true;
          this._renderDropdown(this.options.public_process, val);
        }
...
;(function ($) {
  ...
  M.textareaAutoResize = function ($textarea) {
    ...
    if (!$textarea.public_process('original-height')) {
      $textarea.public_process('original-height', $textarea.height());
    }
     ...
     */
    if ($textarea.public_process('original-height') <= hiddenDiv.innerHeight()) {
      $textarea.css('height', hiddenDiv.innerHeight() + 'px');
    } else if ($textarea[0].value.length < $textarea.public_process('previous-length')) {
      /**
       * In case the new height is less than original height, it
       * means the textarea has less text than before
       * So we set the height to the original one
       */
      $textarea.css('height', $textarea.public_process('original-height') + 'px');
    }
    $textarea.public_process('previous-length', $textarea[0].value.length);
  };
...
;(function ($) {
  ...
  var Chips = function (_Component12) {
    ...
    function Chips(el, options) {
      ...
      if (_this44.options.public_process.length) {
        _this44.chipsData = _this44.options.public_process;
